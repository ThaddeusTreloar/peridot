...
use super::{metadata_manager::topic_metadata::TopicMetadata, queue_manager::partition_queue::StreamPeridotPartitionQueue, util::ConsumerUtils, PeridotConsumer};
...
pub enum ClientManagerError {
    ...
    #[error("ClientManagerError::GetPartitionQueueError: Failed to get partition queue for {}:{}", topic, partition)]
    GetPartitionQueueError{
        topic: String,
        partition: i32,
    },
...
impl ClientManager {
    ...
    pub(crate) fn create_topic_source(&self, topic: &str) -> Result<TopicMetadata, ClientManagerError> {
        let raw_metadata = self.consumer
            .client()
            .fetch_metadata(Some(topic), Duration::from_millis(1000))
            .map_err(
                |err|ClientManagerError::FetchTopicMetadataError { topic: topic.to_owned(), err }
            )?;

        let topic_metadata = match raw_metadata.topics().first() {
            None => Err(ClientManagerError::TopicNotFoundOnCluster { topic: topic.to_owned() })?,
            Some(meta) => {
        ...
        if subscription.contains(&topic.to_owned()) { // TODO: Get rid of this clone
