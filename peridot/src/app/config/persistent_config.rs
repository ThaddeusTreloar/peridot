...
impl PersistentConfig {
    pub(super) fn write_config<W: Write>(&self, writer: &mut W) -> Result<(), PersistentConfigWriteError> {
...
impl TryFrom<&PeridotConfigBuilder> for PersistentConfig {
    ...
    fn try_from(config: &PeridotConfigBuilder) -> Result<Self, Self::Error> {
        if let Some(missing_field) = PERSISTENT_FIELDS.into_iter().find(|value|!config.contains(*value)) {
            Err(PersistentConfigConversionError::InvalidConfigError { missing_field })
        } else {
            let field_values: HashMap<_, _> = PERSISTENT_FIELDS.into_iter()
                .map(|field|(field.to_owned(), config.get(field).to_owned()))
                .collect();

            Ok(
                Self {
                    group_instance_id: config.get(GROUP_INSTANCE_ID).expect("Failed to get field that should exist for PersistentConfig conversion").to_owned()
                }
            )
        }
    }
...
impl From<PersistentConfig> for Vec<(String, String)> {
    fn from(value: PersistentConfig) -> Self {
        let fields = vec![
            ("group.instance.id".to_owned(), value.group_instance_id)
        ];
