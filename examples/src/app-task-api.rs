...
use peridot::app::{AppBuilder, PeridotApp};
use peridot::engine::util::{DeliveryGuaranteeType, ExactlyOnce};
use peridot::init::init_tracing;
use peridot::pipeline::message::sink::PrintSink;
use peridot::pipeline::message::types::{KeyValue, Value};
...
fn map_changes_of_address_to_city(
    ...
    Box::pin(
        app.stream::<String, Json<ChangeOfAddress>>("changeOfAddress")
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let task_b = app
        .task("addressTopic", task_handler)
